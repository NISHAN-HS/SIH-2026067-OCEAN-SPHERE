...
export const CesiumEarth: React.FC<CesiumEarthProps> = ({
  ...
  // Handle Comprehensive Search Queries (Oceans, Seas, Coordinates, Cities, Countries)
  useEffect(() => {
    ...
    let oceanName = 'Arabian Sea';
    let regionId = 'IND_WEST';

    // Comprehensive Ocean & Sea Dictionary
    if (q.includes('arabian')) {
      targetLat = 15.5; targetLon = 68.5; oceanName = 'Arabian Sea'; regionId = 'IND_WEST';
    } else if (q.includes('bengal')) {
      targetLat = 14.0; targetLon = 87.0; oceanName = 'Bay of Bengal'; regionId = 'IND_EAST';
    } else if (q.includes('indian ocean') || q === 'indian') {
      targetLat = -5.0; targetLon = 75.0; altitude = 12000000.0; oceanName = 'Indian Ocean'; regionId = 'IND_SOUTH';
    } else if (q.includes('pacific')) {
      targetLat = 0.0; targetLon = 160.0; altitude = 16000000.0; oceanName = 'Pacific Ocean'; regionId = 'PACIFIC';
    } else if (q.includes('atlantic')) {
      targetLat = 15.0; targetLon = -30.0; altitude = 16000000.0; oceanName = 'Atlantic Ocean'; regionId = 'ATLANTIC';
    } else if (q.includes('red sea')) {
      targetLat = 20.0; targetLon = 38.0; altitude = 2500000.0; oceanName = 'Red Sea'; regionId = 'RED_SEA';
    } else if (q.includes('gulf') || q.includes('persian')) {
      targetLat = 26.0; targetLon = 52.0; altitude = 2000000.0; oceanName = 'Persian Gulf'; regionId = 'GULF';
    } else if (q.includes('andaman')) {
      targetLat = 11.6; targetLon = 92.7; oceanName = 'Andaman Sea'; regionId = 'IND_ANDAMAN';
    } else if (q.includes('gujarat')) {
      targetLat = 21.5; targetLon = 69.5; oceanName = 'Gujarat Coastal Zone'; regionId = 'IND_GUJARAT';
    } else if (q.includes('mumbai')) {
      targetLat = 19.0760; targetLon = 72.8777; altitude = 1500000.0; oceanName = 'Arabian Sea (Mumbai Coast)'; regionId = 'IND_WEST';
    } else if (q.includes('chennai')) {
      targetLat = 13.0827; targetLon = 80.2707; altitude = 1500000.0; oceanName = 'Bay of Bengal (Chennai Coast)'; regionId = 'IND_EAST';
    } else if (q.includes('kochi') || q.includes('cochin')) {
      targetLat = 9.9312; targetLon = 76.2673; altitude = 1500000.0; oceanName = 'Arabian Sea (Kochi Coast)'; regionId = 'IND_WEST';
    } else if (q.includes('kolkata')) {
      targetLat = 22.5726; targetLon = 88.3639; altitude = 1500000.0; oceanName = 'Bay of Bengal (Kolkata Coast)'; regionId = 'IND_EAST';
    } else if (q.includes('colombo') || q.includes('sri lanka')) {
      targetLat = 6.9271; targetLon = 79.8612; altitude = 1500000.0; oceanName = 'Indian Ocean (Sri Lanka Coast)'; regionId = 'IND_SOUTH';
    } else if (q.includes('male') || q.includes('maldives')) {
      targetLat = 4.1755; targetLon = 73.5093; altitude = 1500000.0; oceanName = 'Indian Ocean (Maldives)'; regionId = 'IND_SOUTH';
    } else if (q.includes('dubai') || q.includes('uae')) {
      targetLat = 25.2048; targetLon = 55.2708; altitude = 1500000.0; oceanName = 'Persian Gulf (Dubai)'; regionId = 'GULF';
    } else if (q.includes('muscat') || q.includes('oman')) {
      targetLat = 23.5880; targetLon = 58.3829; altitude = 1500000.0; oceanName = 'Arabian Sea (Muscat)'; regionId = 'IND_WEST';
    } else if (q.includes('singapore')) {
      targetLat = 1.3521; targetLon = 103.8198; altitude = 1500000.0; oceanName = 'Malacca Strait (Singapore)'; regionId = 'PACIFIC';
    } else {
      // Parse coordinates (e.g. 15.42, 68.21 or 15.42 68.21)
      const parts = searchQuery.replace(/[^0-9.-]/g, ' ').trim().split(/\s+/);
      if (parts.length >= 2) {
        ...
        if (!isNaN(parsedLat) && !isNaN(parsedLon)) {
          ...
          targetLon = parsedLon;
          altitude = 2000000.0;
          oceanName = `Coordinates ${targetLat.toFixed(2)}° N, ${targetLon.toFixed(2)}° E`;
        }
      }
    }

    // Fly camera smoothly to search target
    viewerRef.current.camera.flyTo({
      destination: Cesium.Cartesian3.fromDegrees(targetLon, targetLat, altitude),
      duration: 2.0
    });

    // Update selected location metrics for search target
    const relScore = Math.min(99.4, Math.max(55.0, 88 + Math.sin(targetLat * 0.1) * 10));
    const locData: SelectedLocationData = {
      latitude: targetLat,
      longitude: targetLon,
      oceanName: oceanName,
      regionName: `${oceanName} Search Target`,
      regionId: regionId,
      temperature: Number((27.5 + Math.sin(targetLat * 0.2) * 2.5).toFixed(1)),
      salinity: Number((35.1 + Math.cos(targetLon * 0.1) * 0.8).toFixed(1)),
      currentSpeed: Number((0.35 + Math.abs(Math.sin(targetLat)) * 0.3).toFixed(2)),
      currentDirection: Math.floor((targetLat * targetLon * 13) % 360),
      waveHeight: 1.6,
      depth: 2800,
    ...
    // Place pin at search destination
