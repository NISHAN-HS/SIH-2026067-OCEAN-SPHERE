...
export const AlertTable: React.FC<AlertTableProps> = ({ alerts }) => {
  ...
  const getSevBadge = (sev: string) => {
    switch (sev) {
      case 'CRITICAL':
        return { bg: 'bg-rose-100 text-rose-700 border-rose-200', icon: ShieldAlert };
      case 'WARNING':
        return { bg: 'bg-amber-100 text-amber-700 border-amber-200', icon: AlertTriangle };
      default:
        return { bg: 'bg-sky-100 text-sky-700 border-sky-200', icon: Info };
    }
  };
              ...
              filtered.map((alert) => {
                ...
                return (
                  <tr key={alert.alert_id} className={`hover:bg-slate-50 dark:bg-slate-800 transition-colors ${isAck ? 'opacity-60 bg-slate-50 dark:bg-slate-800/50' : ''}`}>
                    ...
                    <td className="p-3 font-semibold text-ocean-600">{alert.region_id}</td>
                    <td className="p-3 text-slate-500 font-mono">{alert.latitude?.toFixed(2)}°N, {alert.longitude?.toFixed(2)}°E</td>
